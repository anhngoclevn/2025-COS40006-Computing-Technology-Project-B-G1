...
if ($row = $result->fetch_assoc()) {
    $alsScore = floatval($row['ALSScore']);
    $totalSec = intval($row['TotalLabeledSeconds']);
    ...
    // Map đúng theo các hành vi trong BEHAVIOR_CLASSES
    $getSec = function ($key) use ($secondsArr) {
        return isset($secondsArr[$key]) ? floatval($secondsArr[$key]) : 0.0;
    };
    ...
    // Không có dữ liệu active cho sinh viên này
    echo json_encode([
        "success" => true,
        "data" => null
    ]);
